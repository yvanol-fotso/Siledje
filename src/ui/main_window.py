# version 1.0.0 améliorée avec corrections de la barre de statut et barre d'outils
...
from PySide6.QtGui import QAction, QActionGroup, QIcon, QCloseEvent, QShortcut, QPixmap, QPainter, QPainterPath
from PySide6.QtCore import Slot, QTimer, QSettings, QTime, QSize, Qt
...
# Utilitaires
from src.core_v1.database import DatabaseManager
from src.core_v1.config import AppConfig
from src.core_v1.notifications import NotificationManager
...
#         self.setFixedSize(300, 200)  # Taille uniforme
#         layout = QVBoxLayout()
#
#         # Formulaire
...
#         self.current_theme_action = None  # Pour cocher le thème sélectionné
#         self.authenticated = False  # État d'authentification pour modules sensibles
#
...
#         else:
#             self.apply_theme(self.config.default_theme)
#
...
#         self.add_permanent_tab("Accueil", self.create_home_tab(), QIcon.fromTheme("go-home"))
#         self.add_permanent_tab("Stock", self.modules['stock'].get_ui(), QIcon.fromTheme("folder"))
#         self.add_permanent_tab("Ventes", self.modules['sales'].get_ui(), QIcon.fromTheme("shopping-cart"))
#         self.add_permanent_tab("Rapports", self.modules['reports'].get_ui(), QIcon.fromTheme("document"))
#         self.add_permanent_tab("Barcode", self.modules['barcode_test'].get_ui(), QIcon.fromTheme("barcode_test"))
#         self.add_permanent_tab("IA", self.modules['ai'].get_ui(), QIcon.fromTheme("system-search"))
#         self.add_permanent_tab("Sécurité & Accès", self.modules['security'].get_ui(), QIcon.fromTheme("dialog-password"))
#         self.add_permanent_tab("Synchronisation Cloud", QWidget(), QIcon.fromTheme("folder-sync"))
#
...
#         sensitive_tabs = [5, 6, 7]  # IA, Sécurité & Accès, Synchronisation Cloud
#         if index in sensitive_tabs and not self.authenticated:
#             self.tab_widget.setCurrentIndex(0)
#             self.show_login_dialog()
#
#     def create_home_tab(self):
#         """Crée l'onglet d'accueil personnalisé"""
#         tab = QWidget()
#         layout = QVBoxLayout()
#         self.welcome_label = QLabel(f"Bienvenue {self.current_user.name}")
#         self.welcome_label.setObjectName("welcome_label")
#         layout.addWidget(self.welcome_label)
#         layout.addWidget(self.create_stats_summary())
#         tab.setLayout(layout)
#         return tab
#
#     def create_stats_summary(self):
#         """Crée un résumé des statistiques (placeholder)"""
#         return QLabel("Statistiques en cours de développement...")
#
#     def setup_menu(self):
#         """Configure le menu complet avec sous-menus"""
#         menubar = self.menuBar()
#         file_menu = menubar.addMenu("&Fichier")
#         login_action = QAction(QIcon.fromTheme("system-run"), "Connexion", self)
...
#         exit_action = QAction(QIcon.fromTheme("application-exit"), "Quitter", self)
...
#             if filename == self.config.default_theme:
#                 action.setChecked(True)
#                 self.current_theme_action = action
#             action.triggered.connect(lambda _, f=filename, a=action: self.apply_theme(f, a))
...
#         """Active ou désactive le mode auto-thème"""
...
#         """Applique un thème selon l'heure"""
...
#         """Recharge le thème par défaut"""
#         self.apply_theme(self.config.default_theme)
#
#     def get_theme_path(self, theme_file):
#         """Construit le chemin absolu vers le thème à partir de son nom"""
#         base_dir = os.path.dirname(os.path.abspath(__file__))
#         return os.path.normpath(os.path.join(base_dir, "../../assets/styles", theme_file))
#
#     def apply_theme(self, theme_name, action=None):
#         """Applique un thème CSS à partir du nom du fichier"""
#         try:
#             theme_path = self.get_theme_path(theme_name)
#             with open(theme_path, "r") as f:
#                 self.setStyleSheet(f.read())
#             self.config.set("default_theme", theme_name)
#             if action:
...
#         toolbar.setIconSize(QSize(24, 24))  # Taille des icônes réduite à 32x32
#
#         # Action Actualiser
#         refresh_action = QAction(QIcon.fromTheme("view-refresh"), "Actualiser", self)
...
#         caisse_quick_action = QAction(QIcon.fromTheme("emblem-sales"), "Caisse", self)
...
#
#         stock_quick_action = QAction(QIcon.fromTheme("folder-stock"), "Stock", self)
...
#         new_sale_action = QAction(QIcon.fromTheme("list-add"), "Nouvelle Vente", self)
...
#
#         # Action Plus
#         more_action = QAction(QIcon.fromTheme("list-more"), "Plus", self)
...
#         self.user_info.setContentsMargins(10, 0, 10, 0)  # Espace entre le texte et le reste
#         toolbar.addWidget(self.user_info)
#
#         # Avatar circulaire (avec masque réel pour arrondi parfait)
#         avatar_label = QLabel()
#         avatar_path = os.path.join(os.path.dirname(__file__), "../../assets/images/avatar.jpeg")
#         avatar_pixmap = QPixmap(avatar_path).scaled(32, 32, Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation)
#
...
#         painter.end()
#
#         # Appliquer l'image masquée
#         avatar_label.setPixmap(masked_pixmap)
#         avatar_label.setFixedSize(32, 32)
#         avatar_label.setStyleSheet("margin-right: 0px;")  # espacement géré par le spacer
#         toolbar.addWidget(avatar_label)
#
#         # Espacement fixe entre avatar et bouton
#         spacer = QWidget()
#         spacer.setFixedWidth(10)
#         toolbar.addWidget(spacer)
#
#         # Bouton de déconnexion vert avec hover rouge
#         logout_button = QPushButton()
#         logout_button.setIcon(QIcon.fromTheme("system-log-out"))
...
#                 )
#             else:
#                 print("⚠️ QLabel 'welcome_label' introuvable.")
...
# V2 test amelioration accueil


# version 2.2.5 améliorée avec intégration de la page d'accueil et corrections
...
#
#         # Onglets permanents avec icônes
#         self.add_permanent_tab("Accueil", self.modules['accueil'].get_ui(), QIcon.fromTheme("go-home"))
#         self.add_permanent_tab("Stock", self.modules['stock'].get_ui(), QIcon.fromTheme("folder-stock"))
#         self.add_permanent_tab("Ventes", self.modules['sales'].get_ui(), QIcon.fromTheme("shopping-cart"))
#         self.add_permanent_tab("Rapports", self.modules['reports'].get_ui(), QIcon.fromTheme("document"))
#         self.add_permanent_tab("Barcode", self.modules['barcode_test'].get_ui(), QIcon.fromTheme("barcode_test"))
#         self.add_permanent_tab("IA", self.modules['ai'].get_ui(), QIcon.fromTheme("system-search"))
...
#         toolbar = self.addToolBar("Outils principaux")
#         toolbar.setMovable(False)
#         toolbar.setIconSize(QSize(24, 24))  # Taille des icônes
#
#         # Action Actualiser
#         refresh_action = QAction(QIcon.fromTheme("view-refresh"), "Actualiser", self)
#         refresh_action.setStatusTip("Actualiser les données de l'onglet actuel")
...
#         logout_button.setFixedSize(32, 32)
#         logout_button.setStyleSheet("""
#             QPushButton { background-color: green; border: none; border-radius: 16px; }
...
class MainWindow(QMainWindow):
    ...
    #     self.tab_widget.currentChanged.connect(self.check_tab_access)
    #
    #     # Onglets permanents avec icônes (Utilise des icônes de thème pour l'exemple)
    #     self.add_permanent_tab("Accueil", self.modules['accueil'].get_ui(), QIcon.fromTheme("go-home", QIcon(self.get_icon_path("home.png"))))
    #     self.add_permanent_tab("Stock", self.modules['stock'].get_ui(), QIcon.fromTheme("folder-stock", QIcon(self.get_icon_path("stock.png"))))
    #     self.add_permanent_tab("Ventes", self.modules['sales'].get_ui(), QIcon.fromTheme("shopping-cart", QIcon(self.get_icon_path("sales.png"))))
    #     self.add_permanent_tab("Rapports", self.modules['reports'].get_ui(), QIcon.fromTheme("document", QIcon(self.get_icon_path("reports.png"))))
    #     self.add_permanent_tab("Barcode", self.modules['barcode_test'].get_ui(), QIcon.fromTheme("barcode_test", QIcon(self.get_icon_path("barcode_test.png"))))
    #     self.add_permanent_tab("IA", self.modules['ai'].get_ui(), QIcon.fromTheme("system-search", QIcon(self.get_icon_path("ai.png"))))
    #     self.add_permanent_tab("Sécurité & Accès", self.modules['security'].get_ui(), QIcon.fromTheme("dialog-password", QIcon(self.get_icon_path("security.png"))))
    #     self.add_permanent_tab("Synchronisation Cloud", QWidget(), QIcon.fromTheme("folder-sync", QIcon(self.get_icon_path("cloud.png"))))
    #
    #     self.setCentralWidget(self.tab_widget)


# je decale le bar code j eme t l'espace entre le haut et le bas
