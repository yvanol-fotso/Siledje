"""
Vue fournisseurs — CustomButton + ThemedTable + Palette.
"""
...
from src.ui.views.supplier.supplier_table import SupplierTable
...
class SupplierView(BaseView):
    ...
    def _init_search(self):
        ...
        add_btn = success_btn(
            ...
            lambda: self.add_supplier_requested.emit(),
        )
    ...
    def _on_edit(self):
        ...
        if row >= 0:
            self.edit_supplier_requested.emit(row)
        else:
            self.show_error(
                "Veuillez selectionner un fournisseur.", "Selection requise"
            )

    def _on_delete(self):
        row = self.table.currentRow()
        if row >= 0:
            self.delete_supplier_requested.emit(row)
        else:
            self.show_error(
                "Veuillez selectionner un fournisseur.", "Selection requise"
            )
    ...
    def set_theme(self, is_dark: bool):
        ...
        try:
            ...
            print(f"[SupplierView] theme table: {e}")
        self._restyle_all_buttons()
        self._apply_theme_styles()
    ...
    def update_suppliers(self, suppliers: list):
        self.table.set_suppliers(suppliers)
        self._last_selected_row = -1
