...
class FileManager(QObject):
    ...
    def import_stock_csv(self, file_path: str):
        try:
            path = Path(file_path)
            if not path.exists():
                QMessageBox.warning(self.view, "Fichier introuvable",
            ...
            with open(path, 'r', encoding='utf-8-sig', newline='') as f:
                ...
                if not reader.fieldnames:
                    QMessageBox.warning(self.view, "CSV vide",
                        "Le fichier CSV est vide ou mal formaté.")
                    return
                ...
                missing = [c for c in ['nom', 'prix', 'quantite'] if c not in col_map]
                if missing:
                    QMessageBox.warning(self.view, "Colonnes manquantes",
                ...
                for row_num, row in enumerate(reader, start=2):
                    try:
                        ...
                        if not nom:
                            errors.append(f"Ligne {row_num}: nom manquant"); continue
                        try:
                            prix = float(prix_str)
                        except ValueError:
                            errors.append(f"Ligne {row_num}: prix invalide '{prix_str}'"); continue
                        try:
                            quantite = int(qty_str)
                        except ValueError:
                            errors.append(f"Ligne {row_num}: quantité invalide '{qty_str}'"); continue

                        cursor.execute("SELECT id FROM products WHERE name = ?", (nom,))
                        if cursor.fetchone():
                            cursor.execute(
            ...
            QMessageBox.critical(self.view, "Erreur d'import", f"Erreur:\n{str(e)}")
    ...
    def export_stock_csv(self, file_path: str):
        try:
            cursor = self.db.db.get_cursor()
            cursor.execute(
                "SELECT name, description, price, quantity, category FROM products ORDER BY name")
            products = cursor.fetchall()

            if not products:
                QMessageBox.information(self.view, "Stock vide",
                    "Il n'y a aucun produit à exporter.")
                return
            ...
            with open(path, 'w', encoding='utf-8-sig', newline='') as f:
                ...
                for p in products:
                    writer.writerow([
                        p[0] or '', p[1] or '',
                        str(p[2]).replace('.', ',') if p[2] else '0',
                        p[3] or 0, p[4] or 'Général',
                    ])
    ...
    def create_backup(self):
        try:
            if not self.db_path.exists():
                QMessageBox.warning(self.view, "Base introuvable",
                    f"Fichier de base de données introuvable:\n{self.db_path}")
                return
            ...
            QMessageBox.information(self.view, "Sauvegarde créée",
    ...
    def restore_backup(self, backup_path: str):
        try:
            ...
            QMessageBox.information(self.view, "Restauration réussie",
                f"Base de données restaurée.\n\nDepuis: {path.name}\n\n"
                f"Sauvegarde de sécurité: {auto_backup.name}\n\n"
    ...
    def delete_backup(self, backup_path: str):
        try:
            ...
            QMessageBox.critical(self.view, "Erreur", f"Erreur:\n{str(e)}")
    ...
    def generate_csv_template(self, file_path: str):
        try:
            ...
            QMessageBox.information(self.view, "Modèle créé",
                f"Modèle CSV créé:\n{path.absolute()}\n\n"
